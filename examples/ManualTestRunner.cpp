...
void testNetworkCallCreation(int &passed, int &failed)
{
    ...
    check(std::holds_alternative<std::monostate>(callCreationTestNetwork.createCall(5, 1, 2)), "Creating first call between user 1 and user 2",
          passed, failed);
    check(callCreationTestNetwork.getCallCount() == 1, "Call count is 1 after first call", passed, failed);
    check(std::holds_alternative<CreateCallError>(callCreationTestNetwork.createCall(6, 4, 2)),
    ...
    check(std::holds_alternative<CreateCallError>(callCreationTestNetwork.createCall(7, 2, 4)),
          "Creating third call between user 2 and not existing user rejected", passed, failed);
    check(std::holds_alternative<CreateCallError>(callCreationTestNetwork.createCall(8, 1, 1)),
    ...
    check(std::holds_alternative<std::monostate>(callCreationTestNetwork.createCall(9, 2, 3)), "Creating call between user 2 and user 3", passed,
          failed);
